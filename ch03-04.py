...
def numpy2mat(arr: np.ndarray) -> List[List[float]]:
...
class DenseLayer:
    def __init__(self, input_count: int, neuron_count: int) -> None:
        ...
        self.weights: List[List[float]] = numpy2mat(np.random.randn(input_count, neuron_count) * 0.01)
        self.outputs: List[List[float]] = numpy2mat(np.zeros((input_count, neuron_count)))
        self.biases : List[float] = list(np.zeros((1, neuron_count)))


    def forward(self, inputs: List[List[float]]) -> List[List[float]]:
        self.outputs = layer(inputs, self.weights, self.biases)
        return self.outputs
